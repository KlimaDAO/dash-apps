...
def deposited_over_time(df):
    ...
    fig.update_layout(paper_bgcolor=colors['bg_color'], plot_bgcolor=colors['bg_color'], font_color='white',
                      xaxis=dict(showgrid=False), yaxis=dict(showgrid=False), hovermode='x unified',
                      hoverlabel=dict(font_color='white', font_size=20), font_size=20)
    return fig
