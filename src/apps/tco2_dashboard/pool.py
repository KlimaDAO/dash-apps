...
def create_pool_content(
    ...
    if retired is None:
        retired_card = dbc.Card(
            [
                html.H5(
                    f"Cumulative Tonnes of {bridge_ticker} Retired from {pool_ticker}",
        ...
        retired_graph = dbc.Card(
            [
                html.H5(
                    f"Cumulative Tonnes Retired from {pool_ticker} Over Time",
                    className="card-title",
                ),
                dbc.CardBody("Coming soon", className="card-text"),
            ],
    ...
    if redeemed["Quantity"].sum() == 0 or redeemed.empty:
        redeemed_graph = dbc.Card(
            [
                html.H5(
                    f"Cumulative Tonnes of {bridge_ticker} Redeemed Over Time",
                    className="card-title",
                ),
                dbc.CardBody("0", className="card-text"),
            ],
            className="card-graph",
        )
    else:
    ...
    def table_type(df_column):
        if [df_column] in ["Vintage", "Quantity"]:
            return "numeric"
        else:
            return "text"
            ...
            dbc.Col(
                dbc.Card(
                    [
                        dbc.CardHeader(
                            html.H1(
                                f"{bridge_name} Carbon Pool: {pool_name} ({pool_ticker})",
